...
for ilag in range(1, int(LAGS_N / LAGS_LENGTH)+1, 1):
...
datafile = 'panel_prepared_distlags_L20.csv'
...
datafile = 'panel_prepared_distlags_L10.csv'
df.to_csv(os.path.join(DATA, datafile), index=False)

cols_drop = [c for c in df.columns if (np.sum(['_L'+str(i) in c for i in range(6, 11, 1)]) > 0.)]
df = df.drop(columns=cols_drop)

datafile = 'panel_prepared_distlags_L05.csv'
df.to_csv(os.path.join(DATA, datafile), index=False)
