...
DATAPATH <- './data/'
RESULTPATH <- './results/'
LABEL <- 'firstdiff_modelselection_L10'
...
weather.all <- c('absh', 'q', 'r', 't2m', 'ssrd', 'tp', 'wbgt', 'utci')
...
## START
df.variables <- read.csv('modelselection_allvariables.csv')
...
for (mobmode in mobmodes) {
	...
	print(mobmode)
	if (mobmode == 'nomob') {
		vars <- c('dlog', weather.lags, weather.lags.sq)
		rows <- rowSums(is.na(df[, vars])) == 0

	} else if (mobmode == 'withoutmob') {
		rows <- (rowSums(is.na(df[, c('dlog', weather.lags)])) == 0) & (rowSums(is.na(df[, mobility.lags])) != 0)
