...
outpath <- "results/epimodel-0614.csv"
weather <- c('absh', 'r', 'tp')
regfilter <- function(rows) (any(!is.na(rows$Deaths)))
...
parameters {
  ...
  real<lower=0, upper=1> deathomega; // rate of reported deaths
...
transformed parameters {
  ...
  for (tt in 2:T) {
    ...
    ddeaths[tt-1] = dcc[tt-1] * deathrate;
  }
}
model {
  // priors
  ...
  deathomega ~ uniform(omega0, 1); // greater than expected confirmed rate
  ...
  ddeaths_true ~ lognormal(ddeaths * deathomega, error);
}"
