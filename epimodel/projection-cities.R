...
ggplot(projdf, aes(date, dcc / max(maxcases, cc1, na.rm=T), linetype=paramregid != '  ')) +
...
ggplot(projdf) +
    geom_hline(aes(yintercept=latitude), colour='green') +
    geom_line(aes(date, latitude + 100 * dcc / max(maxcases, cc1, na.rm=T), group=paste(regid, paramregid), linetype=paramregid == '  ')) +
