...
transformed parameters {
  ...
  for (tt in 2:T) {
    ...
    dcc[tt-1] = 0;
    for (ss in (tt-early0):(tt-early1)) {
      if (tt > 1)
        dcc[tt-1] += new_ee1[ss-1] * omega * portion_early / (early1-early0+1);
    }
    for (ss in (tt-late0):(tt-late1)) {
      if (tt > 1)
        dcc[tt-1] += new_ee1[ss-1] * omega * (1 - portion_early) / (late1-late0+1);
    }
  }
}
model {
