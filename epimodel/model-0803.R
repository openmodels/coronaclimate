...
outpath <- "../../results/epimodel-0803.csv"
weather <- c('r', 't2m', 'tp')
  ...
  for (tt in 2:T) {
    ...
    if (tt - early0 > 1) {
      if (tt - late0 > 1)
        ...
        dcc[tt-1] = omega[tt-1] * (portion_early * sum(new_ee1[max(1, tt-early1-1):(tt-early0-1)]) / (early1-early0+1));
    } else
      dcc[tt-1] = 0;
  }
}
...
for (regid in unique(df$regid)) {
    ...
    resrow <- rbind(get.paramdf(regid, 'alpha', la$alpha, rhats),
                    ...
                    get.paramdf(regid, 'e.absh', la$effect[,1], rhats, rhatparam='effect\\[1\\]'),
                    get.paramdf(regid, 'e.r', la$effect[,2], rhats, rhatparam='effect\\[2\\]'),
                    get.paramdf(regid, 'e.tp', la$effect[,3], rhats, rhatparam='effect\\[3\\]'),
