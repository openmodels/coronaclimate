...
ggplot() +
    geom_line(data=subset(results, time >= 100 & time <= 130 & pulsetime == 100), aes(time - 100, impresp, colour=delaysum, group=delaysum)) +
    facet_grid(effect ~ ., scale="free_y") +
    geom_rect(data=data.frame(effect=c('beta', 'omega')), aes(xmin=1.5, xmax=3.5, ymin=-.1, ymax=.2, fill='one'), alpha=.5) +
    geom_rect(data=data.frame(effect=c('beta', 'omega')), aes(xmin=3.5, xmax=12.5, ymin=-.1, ymax=.2, fill='two'), alpha=.5) +
    geom_rect(data=data.frame(effect=c('beta', 'omega')), aes(xmin=12.5, xmax=30, ymin=-.1, ymax=.2, fill='three'), alpha=.5) +
    scale_x_continuous(name="Time from impulse", expand=c(0, 0)) + theme_bw()
